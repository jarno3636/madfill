...
function isNumeric(x) {
  return (typeof x === 'string' || typeof x === 'number') && String(x).trim() !== '' && /^[0-9]+$/.test(String(x))
}
...
function getApiBase() {
  // In the browser, relative URL is ideal (respects current origin, mini-app webview, previews, etc.)
  if (typeof window !== 'undefined') return ''
  // On server, prefer explicit public site URL (e.g., Vercel env) or fallback to empty (relative).
