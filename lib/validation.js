...
export const createRoundSchema = z.object({
  ...
  // coerce form inputs (strings) into numbers safely
  entryFee: z.coerce.number().min(0.001).max(10),
  duration: z.coerce.number().min(1).max(30),
  blankIndex: z.coerce.number().int().min(0),
})

export const joinRoundSchema = z.object({
  poolId: z.coerce.number().int().min(1),
  word: z.string().min(1).max(16).regex(/^[a-zA-Z0-9\-_]+$/),
  blankIndex: z.coerce.number().int().min(0),
...
export const validateGameWord = (word) => {
  ...
  // keep only allowed chars; do NOT split on '-' or '_' (schema allows them)
  const cleaned = sanitized.replace(/[^a-zA-Z0-9\-_]/g, '')
  // collapse internal spaces (should be none after cleaned); keep a single token by whitespace only
...
export const validateTemplate = (template) => {
  ...
  // ensure parts are strings (non-null/undefined)
  if (!template.parts.every((p) => typeof p === 'string')) {
    throw new Error('All template parts must be strings')
  }
...
export const createRateLimiter = (maxRequests = 5, windowMs = 30000) => {
  ...
  return (key) => {
    ...
    const k = String(key ?? '')
    const arr = (requests.get(k) || []).filter((t) => t > start)
